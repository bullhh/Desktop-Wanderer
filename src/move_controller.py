...
def _current_target_geometry():
    ...
    return target_w, target_h, left, top, right, bottom, target_cx, target_cy, target_position

def move_controller(direction: DirectionControl, result: list[Box]) -> dict[str, float]:
    global _cycle_time, _last_ball_center_x
    target_w, target_h, left, top, right, bottom, target_cx, _, target_position = _current_target_geometry()
    ...
    if result and len(result) > 0:
        ...
        if center_x < left: # 如果桶位于目标框左侧
            ...
            if abs(target_cx - center_x) < target_w:
                ...
                action = direction.get_action("rotate_right")
            _cycle_time = 0
        elif position < target_position * 2.1: # 如果桶在摄像头中的直径小于目标框的2.6倍，则前进
